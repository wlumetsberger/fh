...
/**
 * Helper Class to Generate TableView, and DetailPane
 * out of a Pojo with ViewProperty -> annotations
 * @author Wolfgang
 *
 */
public class Util {
	...
	/**
	 * Generates a DetailDialog for a given Class
	 * editable property defines if the value is editable
	 * @param clazz
	 * @param editable
	 * @return
	 */
	 ...
	 * @param valueHolder
	 */
	public static void writeValuesToDetailPane(Pane p,
			Object valueHolder) {
		if (valueHolder == null) {
			return;
		}
			...
			for (Node n : p.getChildren()) {
				...
				if (n instanceof CheckBox) {
					...
					for (PropertyDescriptor property : Introspector
							...
							.getPropertyDescriptors()) {
						if (property.getName().equals(name)) {
							Object value = property.getReadMethod().invoke(valueHolder);
							if(value != null){
					...
					for (PropertyDescriptor property : Introspector
							...
							.getPropertyDescriptors()) {
						if (property.getName().equals(name)) {
							BaseModel model = (BaseModel) property
					...
					for (PropertyDescriptor property : Introspector
						...
						if (property.getName().equals(name)) {
							if(property.getPropertyType().equals(String.class)){
								property.getWriteMethod().invoke(valueHolder,
										field.getText());
							}else if (property.getPropertyType().equals(Double.class)){
								try{
									Double help = Double.parseDouble(field.getText());
