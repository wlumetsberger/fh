...
public class PersonDaoJdbc implements PersonDao {
	...
	public int getCount() throws DataAccessException {
		...
		try (Connection conn = ds.getConnection();
				...
				ResultSet resultSet = statement
						.executeQuery("select count(id) as count from Person")) {
			if (resultSet.next())
		...
		try (Connection conn = ds.getConnection();
			...
			try (ResultSet resultSet = statement.executeQuery()) {
				while (resultSet.next()) {
					c.add(new Person(resultSet.getInt("id"), resultSet
	...
	public void update(Person p) throws DataAccessException {
		...
		try (Connection conn = ds.getConnection();
				PreparedStatement statement = conn
						.prepareStatement("update Person SET first_name=?, last_name=?, address=?, phone_number=?"
								+ "where id = ?")) {
			statement.setString(1, p.getFirstName());
			statement.setString(2, p.getLastName());
			statement.setString(3, p.getAddress());
