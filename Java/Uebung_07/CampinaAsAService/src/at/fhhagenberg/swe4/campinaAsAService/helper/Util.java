...
/**
 * Helper Class to Generate TableView, and DetailPane out of a Pojo with
...
public class Util {
	...
	/**
	 * For a Model with @ViewProperty Annotation This Method can produce correct
	 * TableModel with all default Fields
	 * 
	 * @param clazz
	 * @return
	 * @return
			...
			if (anno != null) {
				...
				if (prop.showInDefaultModel()) {
					if (f.getType().equals(boolean.class)
						...
						TableColumn<T, LocalDateTime> col = new TableColumn<T, LocalDateTime>(
								prop.name());
						col.setCellValueFactory(new PropertyValueFactory<T, LocalDateTime>(
								f.getName()));
						retVal.add(col);
					} else {
	...
	/**
	 * Generate a TableView for a Given Class
	 * 
	 * @param clazz
	 * @return
	 */
	...
	/**
	 * Generates a DetailDialog for a given Class editable property defines if
	 ...
	 * @return
	 */
	public static <T> GridPane generateDetailDialog(Class clazz,
			boolean editable) {
		...
		for (Field f : fields) {
			...
			if (anno != null) {
				...
				if (f.getType().equals(boolean.class)) {
					...
					b.setId(f.getName());
					b.setDisable(!editable || !prop.editable());
					...
					picker.setDisable(!editable || !prop.editable());
					picker.setId(f.getName());
					grid.setRowIndex(l, i);
					grid.setRowIndex(picker, i);
					grid.setColumnIndex(l, 1);
					grid.setColumnIndex(picker, 2);
					grid.getChildren().addAll(l, picker);
					i++;
				} else if (prop.isComboBox()) {
					Class c = f.getType();
					Method m;
					try {
						m = c.getDeclaredMethod("getDao");
						if (m != null) {
							Dao o = (Dao) m.invoke(c.newInstance(), null);
							...
							grid.setColumnIndex(l, 1);
							grid.setColumnIndex(box, 2);
							grid.getChildren().addAll(l, box);
							i++;
						}
						...
						System.out.println("cannot write value");
					}

				} else if (prop.isTextField()) {
					Label l = new Label(prop.name());
					TextField t = new TextField();
					t.setEditable(editable && prop.editable());
					t.setId(f.getName());
					grid.setRowIndex(l, i);
			...
			for (Node n : p.getChildren()) {
				if (n instanceof CheckBox) {
					...
					for (PropertyDescriptor property : Introspector
							.getBeanInfo(valueHolder.getClass())
							.getPropertyDescriptors()) {
						if (property.getName().equals(name)) {
							box.setSelected((Boolean) property.getReadMethod()
									.invoke(valueHolder));
							break;
						}
					}
	...
	/**
	 * Read Values from DetailPane to ValueHolder Object
	 * 
	 * @param p
	 * @param valueHolder
	 */
			...
			for (Node n : p.getChildren()) {
				if (n instanceof CheckBox) {
					...
					for (PropertyDescriptor property : Introspector
							...
							.getPropertyDescriptors()) {
						if (property.getName().equals(name)) {
							property.getWriteMethod().invoke(valueHolder,
									box.isSelected());
							break;
						}
					}
					...
					for (PropertyDescriptor property : Introspector
						...
						if (property.getName().equals(name)) {
							if (property.getPropertyType().equals(String.class)) {
								property.getWriteMethod().invoke(valueHolder,
										field.getText());
							} else if (property.getPropertyType().equals(
									Double.class)) {
								try {
									Double help = Double.parseDouble(field
											.getText());
									property.getWriteMethod().invoke(
											valueHolder, help);
								} catch (Exception ex) {
									// do nothing value which is unparse able will not be shown on gui
								}
							}
					...
					for (PropertyDescriptor property : Introspector
							...
							.getPropertyDescriptors()) {
						if (property.getName().equals(name)) {
							LocalDateTime help = LocalDateTime.of(
									((LocalDate) picker.getValue()),
									LocalTime.MIN);
							property.getWriteMethod().invoke(valueHolder, help);
							break;
						}
					}
				}
	...
	public static Annotation getAnnotation(String fieldName, Class clazz,
			Class annotationClass) {
		Field[] fields = clazz.getDeclaredFields();
		Annotation ano = null;
		for (Field f : fields) {
			if (f.getName().equals(fieldName)) {
				ano = f.getAnnotation(annotationClass);
				break;
			}
		}
		return ano;
